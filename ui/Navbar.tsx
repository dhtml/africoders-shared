...
import { Bell, Search, User, Menu, X, Sun, Moon, LogIn, LogOut, ChevronDown } from "lucide-react";
...
export function Navbar({ appName, appIcon, links = [], showSearch = true, accent, loginHref = "/login" }: NavbarProps) {
  ...
  // Silent SSO: if this app has no session, attempt a transparent sign-in via
  // identity once per browser session. Identity will auto-redirect back with a
  // switch token if the user is already logged in there, or immediately bounce
  // back (?silent=1) if they're not — avoiding an unwanted login page flash.
  React.useEffect(() => {
    if (authLoading || isAuthenticated) return;
    // Skip auth-related routes to avoid redirect loops
    if (/^\/(login|register|auth|forgot-password|reset-password)(\/|$)/.test(pathname)) return;
    const flag = "silent_sso_attempted";
              ...
              {!authLoading && isAuthenticated && user && (
                <div className="relative ml-1" ref={userMenuRef}>
                  <button
                    ...
                    className="flex items-center gap-2 h-8 px-2 rounded-lg hover:bg-gray-100/80 dark:hover:bg-white/5 transition-all"
                  >
                    <div className="h-7 w-7 rounded-full bg-emerald-600 flex items-center justify-center text-white text-xs font-bold uppercase">
                      {user.name?.charAt(0) || user.email?.charAt(0) || "U"}
                    </div>
                    <span className="hidden sm:inline text-[13px] font-medium text-gray-700 dark:text-gray-300 max-w-[120px] truncate">
                  ...
                  <AnimatePresence>
                    {userMenuOpen && (
                      ...
                      >
                        ...
                        <div className="py-1">
                          <a href="/dashboard" className="flex items-center gap-2.5 px-3 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                ...
                {!authLoading && isAuthenticated && user && (
                  <>
                    <div className="flex items-center gap-3 px-3 py-2.5 border-t border-gray-200/50 dark:border-white/5 mt-2 pt-3">
                      <div className="h-8 w-8 rounded-full bg-emerald-600 flex items-center justify-center text-white text-xs font-bold uppercase">
                    ...
                    <a href="/dashboard" className="flex items-center gap-2 px-3 py-2.5 text-sm font-medium rounded-xl text-gray-500 hover:text-gray-900 hover:bg-gray-50 dark:text-gray-400 dark:hover:text-white dark:hover:bg-white/5 transition-all">
